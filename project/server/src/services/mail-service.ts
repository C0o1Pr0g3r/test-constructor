...
if (SMTP_HOST === "undefined" || isNaN(SMTP_PORT) || USE_FAKE_EMAIL_SENDING === "undefined" || CLIENT_URL === "undefined") {
    throw new Error("'SMTP_HOST', 'SMTP_PORT', 'USE_FAKE_EMAIL_SENDING' and/or 'CLIENT_URL' not specified in the config file '.env'.");
}
...
class MailService {
    ...
    async sendActivationMail(to: User["email"], activationLink: User["activationLink"]): Promise<void> {
        const pathToLetterTemplate = path.resolve(PATH_TO_FOLDER_WITH_LETTER_TEMPLATES, "activation-letter.pug");
        ...
        if (USE_FAKE_EMAIL_SENDING === "no") {
            ...
            const pathToActivationLetter = path.resolve(pathToFolderWithActivationLetters, `${to.replace("@", "---")}.html`);
